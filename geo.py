...
def separation(latd1, lond1, latd2, lond2):
    print(timing)
    ...
    # Convert to radians
    lat1, lon1 = np.radians(latd1), np.radians(lond1)
    lat2, lon2 = np.radians(latd2), np.radians(lond2)

    d = r*np.arccos(np.cos(lat1)*np.cos(lat2)*np.cos(lon2-lon1) + \
                 np.sin(lat1)*np.sin(lat2))


    coslat1, coslat2 = np.cos(lat1), np.cos(lat2)
...
def separation_matrix(lat1, lon1, lat2, lon2):
...
def cart2geo(latd1, lond1, qdr, dist):
